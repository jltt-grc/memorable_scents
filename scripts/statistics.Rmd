...
📌 **Important:** The condition selection is done in the step **[Condition Selection](#condition-selection)**, where the user must specify the appropriate condition before proceeding with the analysis.  

# 📑 Table of Contents  

- **[Import the Dataset](#import-the-dataset)**  
  - *Load the dataset for analysis.*  
  - [Dataset by Trial](#dataset-by-trial) – *Explore the dataset at the trial level.*  
  - [Dataset by Subject](#dataset-by-subject) – *Aggregate data at the participant level.*  
  - [Condition Selection](#condition-selection) – *Choose the condition of interest for the analysis.*  
  - [Variable Distribution](#variable-distribution) – *Visualize the distribution of key variables.*  
- **[Perceptual Scores](#perceptual-scores)**  
  *Analyze perceptual-related measures across conditions.*  
- **[Gender × Memory Scores](#gender-memory-scores)**  
  *Examine how memory scores vary across genders.*  
- **[Gender × Features](#gender-features)**  
  *Explore relationships between gender and all the features.*  
- **[Memory Score × Features](#memory-score-features)**  
...
```{r plot target rating variability, fig.height=15, fig.width=30}
...
# Create a custom labeller for Perceptual_variable
custom_labeller <- c(
...
ggplot(df_long, aes(x = odor_name, y = Rating)) +
  ...
  facet_grid(
    ~ Perceptual_variable,
    scales = "free_x",
    labeller = labeller(Perceptual_variable = custom_labeller)
    ) +
  custom_theme() +
  labs(
    x= NULL,
...
max_sd <- c(
  pleasantness = 5,
  emotional_strength = 2.5,
  intensity = 5,
  familiarity = 5
)

# SD per odor for targets
sd_target <- df_target %>%
  group_by(odor_name) %>%
  summarise(
    pleasantness = sd(pleasantness, na.rm = TRUE),
    emotional_strength = sd(emotional_strength, na.rm = TRUE),
    intensity = sd(intensity, na.rm = TRUE),
    familiarity = sd(familiarity, na.rm = TRUE),
    .groups = "drop"
  )

# SD averages
mean_sd_target <- colMeans(sd_target[, -1], na.rm = TRUE)

# SD per odor for hits
sd_hit <- df_hit %>%
  group_by(odor_name) %>%
  summarise(
    pleasantness = sd(pleasantness, na.rm = TRUE),
...
mean_sd_hit <- colMeans(sd_hit[, -1], na.rm = TRUE)

# Create the final table
final_summary <- tibble(
  variable = names(max_sd),
  theoretical_sd = unname(max_sd),
  target_sd = mean_sd_target,
  ratio_target = (target_sd / theoretical_sd) * 100,
  hit_sd = mean_sd_hit,
  ratio_hit = (hit_sd / theoretical_sd) * 100
)

# Display the final table
datatable(
  final_summary %>% mutate(across(where(is.numeric), round, 2)),
  caption = "Means of standard deviations by variable and ratios (vs. theoretical SD)",
  options = list(dom = 't')
)
```
